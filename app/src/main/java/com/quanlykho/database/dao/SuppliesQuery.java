...
public class SuppliesQuery implements DAO.SuppliesQuery{
	...
	@Override
	public void createSupplies(Supplies supplies, QueryResponse<Boolean> response){
	...
	public void updateSupplies(Supplies supplies, QueryResponse<Boolean> response){
		...
		if(row>0){
			response.onSuccess(true);
			Toast.makeText(App.context, "Xác nhận thay đổi thông tin nhà cung cấp", Toast.LENGTH_LONG).show();
		}
		else{
	...
	public void deleteSupplies(int suppliesID, QueryResponse<Boolean> response){
		...
		if(row>0){
			response.onSuccess(true);
			Toast.makeText(App.context, "Đã xóa nhà cung cấp", Toast.LENGTH_LONG).show();
		}
		else{
