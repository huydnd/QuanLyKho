package com.quanlykho.constant;

public class DropTable {
	public static String DROP_WAREHOUSE_TABLE
			= "DROP TABLE IF EXISTS " + Constants.WAREHOUSE_TABLE;

	public static String DROP_SUPPLIES_TABLE
			= "DROP TABLE IF EXISTS " + Constants.SUPPLIES_TABLE;

	public static String DROP_RECEIPT_TABLE
			= "DROP TABLE IF EXISTS " + Constants.RECEIPT_TABLE;

	public static String DROP_DETAIL_TABLE
			= "DROP TABLE IF EXISTS " + Constants.DETAIL_TABLE;
}
