					...
					new AlertDialog.Builder(WarehouseActivity.this)
							...
							.setPositiveButton("Xóa", new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									delWarehouse(warehouseArrayList.get(warehouseIndex));
								}
							})
			...
			public boolean onNavigationItemSelected(@NonNull MenuItem menuItem) {
				...
				switch (id) {
					...
					case R.id.nav_statistics:
						startActivity( new Intent(WarehouseActivity.this, SuppliesChartActivity.class));
						break;
					case R.id.nav_pdf:
						startActivity( new Intent(WarehouseActivity.this, CreatePdfActivity.class));
						break;
					case R.id.nav_quit:
						finishAffinity();
						break;
...
//						.map(Warehouse::getWarehouseName).collect(Collectors.toList());
	...
	void delWarehouse(Warehouse warehouse){
		receiptQuery.readAllReceiptFromWarehouse(warehouse.getWarehouseId(), new QueryResponse<List<Receipt>>() {
			@Override
			public void onSuccess(List<Receipt> data) {
				for(Receipt receipt:data){
					detailQuery.deleteDetailByReceiptId(receipt.getReceiptId(), new QueryResponse<Boolean>() {
						@Override
						public void onSuccess(Boolean data) {
						}
						@Override
		...
		exportQuery.readAllExportFromWarehouse(warehouse.getWarehouseId(), new QueryResponse<List<Export>>() {
			@Override
			public void onSuccess(List<Export> data) {
				for(Export export:data){
					exDetailQuery.deleteExDetailByExportId(export.getExportId(), new QueryResponse<Boolean>() {
						@Override
						public void onSuccess(Boolean data) {
						}
						@Override
		...
		receiptQuery.deleteReceiptByWarehouseId(warehouse.getWarehouseId(), new QueryResponse<Boolean>() {
			@Override
			public void onSuccess(Boolean data) {
			}
			@Override
		...
		exportQuery.deleteExportByWarehouseId(warehouse.getWarehouseId(), new QueryResponse<Boolean>() {
			@Override
			public void onSuccess(Boolean data) {
			}
			@Override
		...
		warehouseQuery.deleteWarehouse(warehouse.getWarehouseId(), new QueryResponse<Boolean>() {
			@Override
			public void onSuccess(Boolean data) {
				warehouseArrayList.removeAll(warehouseArrayList);
				warehouseNameList.removeAll(warehouseNameList);
